...
class LinearRegression(BasicLinearModel):
    ...
    **Regularization**
    ...
    >>> model = LinearRegression(learning_rate=0.01, max_iter=1000, penalty=None)
    >>> model.fit(X, y)
    >>> print(f"Coef: {model.coef_}")
    >>> print(f"Intercept: {model.intercept_:.3f}")
    >>>
    >>> # Fit with L2 (Ridge) regularization
    >>> ridge = LinearRegression(learning_rate=0.01, alpha=0.1, penalty='l2')
    >>> ridge.fit(X, y)
    >>>
    >>> # Make predictions
    ...
    def param_update(self, X: npt.NDArray[np.float64], error: npt.NDArray[np.float64]) -> None:
        """
