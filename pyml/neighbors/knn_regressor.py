...
class KNNRegressor:
    ...
    **curse of dimensionality**: distances become increasingly uniform
    ...
    def __predict_regression(self, neighbor_ind: npt.NDArray[np.intp], neighbor_dist: npt.NDArray[np.float64]) -> npt.NDArray[np.float64]:
        """
        ...
        y_pred = cast(
            npt.NDArray[np.float64, ], np.sum(self.y_[neighbor_ind] * weights, axis=1) / np.sum(weights, axis=1)
        )
        return y_pred
