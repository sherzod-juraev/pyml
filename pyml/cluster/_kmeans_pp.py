...
class KmeansPP:
    ...
    **Algorithm:**
    ...
    def __cal_probab(self, centroids_: npt.NDArray[np.float64], X: npt.NDArray[np.float64]) -> npt.NDArray[np.float64]:
        """
        ...
        probability = cast(
            npt.NDArray[np.float64], min_distances / (np.sum(min_distances) + 1e-12)
        )
        return probability
